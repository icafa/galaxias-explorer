        ...
        $scope.init=function()
        {
            ...
            if($scope.txId!==undefined) { // add a test to check if it match tx paterns to avoid useless API call, clients are not obliged to come from the search form...
                ...
                getTransactionInfos()
                    ...
                    $scope.gasPrice = web3.fromWei(result.gasPrice, "ether").toFormat(10) + " GLX";
                    $scope.hash = result.hash;
                    $scope.input = result.input; // that's a string

		    var hex = result.input.toString();
		    var resstr = '';
		    for (var i = 0; i < hex.length; i += 2) resstr += String.fromCharCode(parseInt(hex.substr(i, 2), 16));
		    $scope.inputASCII = resstr;
                    ...
                    $scope.txprice = web3.fromWei(result.gas * result.gasPrice, "ether") + " GLX";
